...
#+SUBTITLE: for version 2.11.0 (2.11.0-459-gcf60b1039+1)
...
#+TEXINFO: @noindent
This manual is for Magit version 2.11.0 (2.11.0-459-gcf60b1039+1).
...
The following commands do something for the selected modules.  If no
module is selected, then they act on the module at point.  If there is
no module at point, then they prompt for a module.  With a prefix
argument they act on all suitable modules.

- Key: o r, magit-submodule-register

  This command registers the chosen module(s) by copying the url from
  ".gitmodules" to "$GIT_DIR/config".  These values can then be edited
  before running ~magit-submodule-populate~.  If you don't need to edit
  the url, then use the latter directly.

- Key: o p, magit-submodule-populate

  This command creates the working directory or directories of the
  chosen module(s), checking out the recorded commit(s).

- Key: o u, magit-submodule-update

  This command updates the chosen module(s) checking out the recorded
  commit(s).

- Key: o s, magit-submodule-synchronize

  This command synchronizes the url(s) of the chosen module(s),
  copying the value(s) from ".gitmodules" to the ".git/config" of the
  super-project as well those of the module(s).

- Key: o d, magit-submodule-unpopulate

  This command removes the working directory of the chosen module(s).

The following commands do something with all modules.
